...
data_path = "data/experiment_3/human/data_0"
```
...
mturk_results = read_csv(file.path(data_path, "mturk_results.csv"), col_names=TRUE) %>%
...
model_0 = read_csv("data/experiment_3/model/predictions/Manhattan/data.csv")

# Perform some basic preprocessing and then z-score the model predictions.
model_1 = model_0 %>%
  dplyr::select(-l_1, -l_2) %>%
  rename(model=p_2) %>%
...
annotation_1 = data.frame(
  label = paste("r = 0.80 (95% CI: 0.36 - 0.94)", sep=""),
  x = c(0.6),
  y = c(0.8)
)
...
figure_2b = data_4 %>%
  ggplot(aes(x=model, y=mean_human, label=map)) +
  geom_point(aes(fill=substr(map, 0, 2)), color="black", pch=21, size=1.5, alpha=0.6) +
  # geom_text(vjust=-1.0) +
  geom_smooth(method="lm", se=TRUE, linetype="dashed", color="black") +
...
# Compute the bootstrapped 95% CIs.
ci = compute_bootstrap(data_4)
ci[4]
ci[5]
```

The Pearson correlation in the above plot is `r cor(na.omit(data_4)$mean_human, na.omit(data_4)$model, method="pearson")`.
