...
One participant used their worker ID as the survey code. `UN_NX_0` also had the wrong corner labels.

```{r eval=FALSE, include=FALSE}
# Read in the participant data (after manually removing errors).
data_0 <- read_csv("data/experiment_1/pilot_1/raw_data.csv", quote="~")

# Read in the MTurk results file.
mturk_results = read_csv("data/experiment_1/pilot_1/mturk_results.csv", col_names=TRUE) %>%
  mutate(Answer.surveycode=substr(Answer.surveycode, 3, length(Answer.surveycode)))
...
for (p in 1:length(data_1)) {
  ...
  data_2 = data_1[p][[1]]$trials %>%
    as.data.frame() %>%
    mutate(map=gsub("_no-grid.png", "", map), unique_id=data_0$UniqueId[p])
...
# Write the preprocessed data.
# write_csv(data_3, "data/experiment_1/pilot_1/data.csv")
```
...
The correlation is `r cor(data_4$z_model, data_4$mean_z_human, method="pearson")`. If we break down the results by goal, we get:
...
# Extract the maps where the model predictions and human judgments differ
# by more than 1 standard deviation.
unique(data_4$map[abs(data_4$mean_z_human-data_4$z_model[,1]) > 1])
```
...
The Pearson correlation is `r cor(data_6$z_model, data_6$mean_z_human, method="pearson")`.If we break down the results by entrance, we get:
...
```{r, echo=FALSE}
# Read in the participant data (after manually removing errors).
data_0 <- read_csv("data/experiment_1/pilot_2/raw_data.csv", quote="~")
data_0 <- read_csv("C:/Users/Michael Lopez-Brau/Downloads/joan.csv", quote="~")
...
# Write the preprocessed data.
# write_csv(data_3, "data/experiment_1/pilot_2/data.csv")
```
...
data_3 %>%
  # rbind(joan) %>%
  select(map, A, B, C, unique_id, wrong_attempts) %>%
  filter(wrong_attempts < 2) %>%
  gather(goal, value, A, B, C) %>%
  # filter(map=="DX_UN_0") %>%
  # filter(map=="ND_PX_0") %>%
  filter(map=="ND_PX_1") %>%
  ggplot(aes(x=goal, y=value)) + 
  geom_point() + 
...
```{r, fig.align="center"}
# Read in the goal judgments and predictions.
model_0 = read_csv("data/model/predictions/Manhattan_current/goal_predictions.csv")
...
# Plot the goal comparison.
data_4 %>%
  # filter(!(map %in% c("DX_UN_0", "ND_PX_0", "ND_PX_1"))) %>%
  # filter(!(map %in% c("DX_UN_0", "ND_PX_1"))) %>%
  ggplot(aes(x=z_model, y=mean_z_human, label=map)) +
  geom_point() +
  # geom_text(vjust=-1.0) +
