...
pilot_path = "data/experiment_1/pilot_5"

# Read in the participant data (after manually removing errors).
data_0 <- read_csv(file.path(pilot_path, "ImageInference_Exp1.csv"), quote="~")
...
data_4 = data_3 %>%
  ...
  # filter(!(unique_id %in% c("3bcOC7j", "moLWXPA", "PoNIgep", "qfVE6hb", "yYCyou3"))) %>%
  filter(!(unique_id %in% c("3bcOC7j", "moLWXPA", "OEdznH0", "PoNIgep", "qfVE6hb", "S9Y8JeP", "VHNf4qQ", "yYCyou3"))) %>%
...
data_6 = data_3 %>%
  filter(wrong_attempts < 2) %>%
  # filter(!(unique_id %in% c("3bcOC7j", "moLWXPA", "PoNIgep", "qfVE6hb", "yYCyou3"))) %>%
  filter(!(unique_id %in% c("3bcOC7j", "moLWXPA", "OEdznH0", "PoNIgep", "qfVE6hb", "S9Y8JeP", "yYCyou3"))) %>%
  filter(!grepl("ND", map)) %>%
...
The Pearson correlation is `r cor(data_7$z_model, data_7$mean_z_human, method="pearson")`. If we split up the data by trial, we get:
...
Start putting stimuli here with:
1. Model predictions
2. Participant judgments

![Caption](../../stimuli/experiment_1/DX_DX_0.png)
