...
fullposterior <- do.call(bind_rows,lapply(StatePredictions$Files,function(x){mutate(read_csv(paste("data/model/predictions/Manhattan/",x,sep=""),col_types=cols()),Trial=x)}))
...
write.csv(Entrance_posterior, "data/model/predictions/Manhattan/entrance_predictions.csv", 
          quote=F, row.names = F)

# Combines the goal prediction files for the diagonal paths.
StatePredictions <- data.frame(Files = list.files("data/model/predictions/diagonal/")) %>%
  mutate(States=str_detect(Files,"States_Posterior")) %>% tbl_df %>% filter(States)

fullposterior <- do.call(bind_rows,lapply(StatePredictions$Files,function(x){mutate(read_csv(paste("data/model/predictions/diagonal/",x,sep=""),col_types=cols()),Trial=x)}))
...
write.csv(Entrance_posterior, "data/model/predictions/diagonal/entrance_predictions.csv", 
          quote=F, row.names = F)
